...
namespace PersonalFinance.Api.Services.Contracts
{
    ...
    {
        Task<IEnumerable<Expense>> GetAllAsync(Guid userId, CancellationToken cancellationToken = default);
        Task<Expense?> GetByIdAsync(int id, Guid userId, CancellationToken cancellationToken = default);
